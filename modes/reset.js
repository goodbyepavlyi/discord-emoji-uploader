...
module.exports = async (arguments) => {
    ...
    for (const guildId of guilds) {
        console.log(`Switching to guild ${colors.bgGrey(guildId)}!`)
        ...
        for (const emoji of emojis) {
            await deleteEmoji(emoji, guildId, token).then(async (response) => {
                if (response.status == 204) {
                    console.log(`Emoji with an ID ${colors.bgGrey(emoji)} has been sucessfully deleted from ${colors.bgGrey(guildId)}!`)
                    await wait(500)
